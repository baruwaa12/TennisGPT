...
class MatchHistoryService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    if (kDebugMode) {
      print(
          'MatchHistoryService: Loaded ${_cachedMatches?.length ?? 0} matches for user');
    }
  }
  ...
  Future<List<MatchPerformance>> getAllMatches() async {
    ...
    try {
      final List<dynamic> matchesList = json.decode(matchesJson);
      final matches =
          matchesList.map((json) => MatchPerformance.fromJson(json)).toList();
  ...
  // Save or update by id to prevent duplicate entries
  Future<void> saveOrUpdateMatch(MatchPerformance match) async {
    final prefs = await SharedPreferences.getInstance();
    final List<MatchPerformance> matches = await getAllMatches();

    final int index = matches.indexWhere((existing) => existing.id == match.id);
    if (index == -1) {
      matches.add(match);
    } else {
      matches[index] = match;
    ...
    final List<Map<String, dynamic>> matchesJson =
        matches.map((item) => item.toJson()).toList();
  ...
  Future<void> updateMatch(MatchPerformance updatedMatch) async {
    ...
    final int index =
        matches.indexWhere((match) => match.id == updatedMatch.id);
    if (index != -1) {
  ...
  Future<List<MatchPerformance>> getMatchesBySurface(String surface) async {
    final List<MatchPerformance> matches = await getAllMatches();
    return matches
        .where((match) => match.surface.toLowerCase() == surface.toLowerCase())
        .toList();
  }
  ...
  Future<double> getWinRate() async {
    ...
    final int wins =
        matches.where((match) => match.result.toLowerCase() == 'win').length;
